...
class FIDEvaluator(BaseEvaluator):
    ...
    def single_evaluate(self, results):
        ...
        mean = np.mean(pred_motion_emb, axis=0)
        std = np.std(pred_motion_emb, axis=0)
        std[std == 0] = 1e-8
        pred_motion_emb = (pred_motion_emb - mean) / std
        
        mean = np.mean(gt_motion_emb, axis=0)
        std = np.std(gt_motion_emb, axis=0)
        std[std == 0] = 1e-8
        gt_motion_emb = (gt_motion_emb - mean) / std
