...
class MatchingScoreEvaluator(BaseEvaluator):
    ...
    def single_evaluate(self, results):
        ...
        with torch.no_grad():
            ...
            mean = np.mean(word_emb, axis=0)
            std = np.std(word_emb, axis=0)
            std[std == 0] = 1e-8
            word_emb = (word_emb - mean) / std
            
            mean = np.mean(motion_emb, axis=0)
            std = np.std(motion_emb, axis=0)
            std[std == 0] = 1e-8
            motion_emb = (motion_emb - mean) / std
