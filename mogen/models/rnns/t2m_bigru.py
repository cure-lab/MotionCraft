...
class T2MMotionEncoder(nn.Module):
    ...
    def __init__(self, input_size, movement_hidden_size, movement_latent_size,
    ...
    def forward(self, motion, motion_length, motion_mask):
        ...
        motion: [B, T, D]
        '''
        print(f"motion encoder input: {motion.shape}")
        print(f"motion encoder motion_length input: {motion_length.shape}")
        ...
        print(f"motion encoder output: {motion_embedding_ordered.shape}")
    ...
    def forward(self, text, token, device):
        ...
        for i in range(B):
            ...
            if len(tokens) < self.max_text_len:
                ...
                tokens = tokens + \
                    ['unk/OTHER'] * (self.max_text_len + 2 - batch_sent_len)
            else:
        ...
        print(f"Text encoder output: {text_embedding.shape}")
...
class TextEncoderBiGRUCo(nn.Module):
    ...
    def __init__(self, word_size, pos_size, hidden_size, output_size):
        ...
        self.output_net = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True), nn.Linear(hidden_size,
                                                       output_size))
    ...
    def forward(self, word_embs, pos_onehot, cap_lens):
        ...
        cap_lens = cap_lens.data.tolist()
        emb = pack_padded_sequence(input_embs,
                                   cap_lens,
                                   batch_first=True,
...
@SUBMODULES.register_module()
class T2MContrastiveModel(BaseModule):

    def __init__(self, motion_encoder=None, text_encoder=None, init_cfg=None):
        super().__init__()
        self.motion_encoder = T2MMotionEncoder(**motion_encoder)
        self.text_encoder = T2MTextEncoder(**text_encoder)
    ...
    def encode_motion(self,
                      ...
                      motion_mask=None,
                      **kwargs):
        return self.motion_encoder(motion, motion_length, motion_mask)

    def encode_text(self, text, token=None, device=None, **kwargs):
        return self.text_encoder(text=text, token=token, device=device)

    def load_pretrained(self, ckpt_path):
        self.motion_encoder.load_pretrained(ckpt_path)
        self.text_encoder.load_pretrained(ckpt_path)
