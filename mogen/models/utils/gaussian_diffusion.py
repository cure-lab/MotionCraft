...
from .scheduler import get_schedule_jump, get_schedule_jump_paper, get_schedule_jump_cjm_ddim
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.opt = opt
        # print(f"GaussianDiffusion: {self.opt}")
    ...
    def p_mean_variance(self,
        ...
        ### CJM: repaint in ddim START ###
        if self.model_mean_type == ModelMeanType.START_X \
                and 'outpainting_mask' in model_kwargs['y'].keys() \
                and 'gt' in model_kwargs['y'].keys() \
                and True in model_kwargs['y']['outpainting_mask']:
            outpainting_mask, inpainted_motion = model_kwargs['y']['outpainting_mask'], model_kwargs['y']['gt']
            assert self.model_mean_type == ModelMeanType.START_X, 'This feature supports only X_start pred for now!'
            assert model_output.shape == outpainting_mask.shape == inpainted_motion.shape
            model_output = (model_output * ~outpainting_mask) + (inpainted_motion * outpainting_mask)
        ### CJM: repaint in ddim END ###
    ...
    def ddim_sample(
        ...
        ### CJM: repaint in ddim START ###
        x = sample # temp
        if 'outpainting_mask' in model_kwargs['y'].keys() and True in model_kwargs['y']['outpainting_mask']:
            gt_keep_mask = model_kwargs['y'].get('outpainting_mask')
            noise_weight = th.sqrt((1 - alpha_bar_prev))

            if self.opt.same_overlap_noisy and model_kwargs['y']['clip_idx'] > 0:
                weighed_gt = model_kwargs['y']['gt']
                weighed_gt[:, :self.opt.overlap_len, ...] = model_kwargs['y']['previous_noisy_tail'][str(t)]
            else:
                gt = model_kwargs['y']['gt']
                gt_weight = th.sqrt(alpha_bar_prev)
                gt_part = gt_weight * gt
                noise_part = noise_weight * th.randn_like(x)
                weighed_gt = gt_part + noise_part

            # if noise_weight[0,0,0] < 0.1 and self.opt.addBlend: # add interpolation at the last step
            if noise_weight[0,0,0] < 0.2 and self.opt.addBlend: # add interpolation at the last steps
                linear_weight = th.linspace(0, 1, self.opt.overlap_len, device=x.device)
                linear_weight = linear_weight.view(1, -1, 1).expand(x.shape[0], -1, -1)
                weighed_gt[:, :self.opt.overlap_len, :] = weighed_gt[:, :self.opt.overlap_len, :] * (1 - linear_weight) + x[:, :self.opt.overlap_len, :] * linear_weight

            x = (weighed_gt * gt_keep_mask) + (x * ~gt_keep_mask)

        if self.opt.same_overlap_noisy:
            # print(t)
            self.saved_noisy_tail[str(t)] = x[..., -self.opt.overlap_len:, :].clone()
    ...
    def ddim_sample_loop(
        ...
        if 'outpainting_mask' in model_kwargs['y'].keys() and True in model_kwargs['y']['outpainting_mask'] and not self.opt.no_repaint:
            for sample in self.ddim_sample_loop_progressive_harmonize(
                model,
                shape,
                noise=noise,
                clip_denoised=clip_denoised,
        ...
        if self.opt.same_overlap_noisy:
            final["saved_noisy_tail"] = self.saved_noisy_tail
            return final
        else:
            return final["sample"]
    ...
    def ddim_sample_loop_progressive(
        ...
        for i in indices:
            ...
            with th.no_grad():
                ...
                yield out
                img = out["sample"]
    def ddim_sample_loop_progressive_harmonize(
        self,
        model,
        shape,
        noise=None,
        ...
        if self.opt.no_resample:
            times = get_schedule_jump_cjm_ddim(int(self.opt.timestep_respacing[4:]))
        else:
            # times = get_schedule_jump_cjm_ddim(int(self.opt.timestep_respacing[4:]), jump_length=3, jump_n_sample=2)
            # times = get_schedule_jump_cjm_ddim(int(self.opt.timestep_respacing[4:]), jump_length=3, jump_n_sample=5)
            times = get_schedule_jump_cjm_ddim(int(self.opt.timestep_respacing[4:]), jump_length=self.opt.jump_length, jump_n_sample=self.opt.jump_n_sample)
        # print(times)
        ...
        for t_last, t_cur in time_pairs:
            # idx_wall += 1
            t_last_t = th.tensor([t_last] * shape[0], device=device) # pylint: disable=not-callable

            if t_cur < t_last:  # denose
                with th.no_grad():
                ...
                image_after_step = self.undo(
                    ...
                    est_x_0=out['pred_xstart'], t=t_last_t+t_shift, debug=False) # TODO: t_shift, q_sample
...
class SpacedDiffusion(GaussianDiffusion):
    ...
    def __init__(self, use_timesteps, **kwargs):
        ...
        # print(f'SpacedDiffusion-use_timesteps: {self.use_timesteps}')
        self.timestep_map = []
