...
class Crop(object):
    ...
    def __init__(self, crop_size: Optional[Union[int, None]] = None, stride: Optional[Union[int, None]] = None):
    ...
    def __call__(self, results):
        ...
        if length >= self.crop_size:
            ...
            if idx > 0 and self.stride is not None:
                idx = idx - idx % self.stride
...
class ContrlCrop(object):
    ...
    def __call__(self, results):
        ...
        if length >= self.crop_size:
            ...
            D = c.shape[1:]
            padding_zeros = np.zeros((padding_length, *D), dtype=np.float32)
            c = np.concatenate([c, padding_zeros], axis=0)
        assert len(motion) == self.crop_size
        assert len(c) == self.crop_size
        results['motion'] = motion
        results['c'] = c
        results['motion_shape'] = motion.shape
        if length >= self.crop_size:
