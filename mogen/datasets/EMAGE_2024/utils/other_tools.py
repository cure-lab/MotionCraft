...
import cv2
import mogen.datasets.EMAGE_2024.utils.media
import mogen.datasets.EMAGE_2024.utils.fast_render
...
def render_one_sequence(
    ...
    silent_video_file_path = mogen.datasets.EMAGE_2024.utils.fast_render.generate_silent_videos(args.render_video_fps, 
    ...
    mogen.datasets.EMAGE_2024.utils.media.add_audio_to_video(silent_video_file_path, audio_path, final_clip)
    os.remove(silent_video_file_path)

def render_one_sequence_wo_gt(
         res_npz_path,
         output_dir,
         model_folder="/data/datasets/smplx_models/",
         model_type='smplx',
    ...
    silent_video_file_path = mogen.datasets.EMAGE_2024.utils.fast_render.generate_silent_videos_no_gt(args.render_video_fps, 
                                                                args.render_video_width,
                                                                args.render_video_height,
                                                                args.render_concurrent_num,
                                                                args.render_tmp_img_filetype,
                                                                ...
                                                                faces,
                                                                output_dir,
                                                                video_name=res_npz_path.split('res_')[1].split('.npz')[0],
                                                                )
    
    #final_clip = f"{output_dir}{res_npz_path.split('/')[-1][4:-4]}.mp4"
    base_filename_without_ext = os.path.splitext(os.path.basename(res_npz_path))[0]
    final_clip = os.path.join(output_dir, f"{base_filename_without_ext}.mp4")
    print(f"Motion Video has been saved in {final_clip}")

def render_one_sequence_wo_audio(
         res_npz_path,
         gt_npz_path,
         output_dir,
         model_folder="/data/datasets/smplx_models/",
