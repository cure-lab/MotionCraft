...
'''
delete foot contact line 329
bert word_embedding -> clip line 421
delete rank line 36 113
'''
class CustomDataset(Dataset):
    def __init__(self, args, loader_type, augmentation=None, kwargs=None, build_cache=True):
        ...
        # print("Debuging")
        # self.selected_file = self.selected_file.iloc[0:2]
        print('self.selected_file: ', self.selected_file)
    ...
    def cache_generation(self, out_lmdb_dir, disable_filtering, clean_first_seconds,  clean_final_seconds, is_test=False):
        ...
        if not os.path.exists(out_lmdb_dir): 
            print(out_lmdb_dir, os.path.exists(out_lmdb_dir))
            os.makedirs(out_lmdb_dir)
        if len(self.args.training_speakers) == 1:
            dst_lmdb_env = lmdb.open(out_lmdb_dir, map_size= int(1024 ** 3 * 20))# 50G
        else:
